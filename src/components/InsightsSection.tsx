...
import { Clock, ArrowUpRight, BookOpen, X, Check } from 'lucide-react';

export const InsightsSection: React.FC = () => {
  const [selectedArticle, setSelectedArticle] = useState<InsightPost | null>(null);

  return (
    <section id="insights" className="py-24 bg-slate-900 text-slate-100 border-b border-slate-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {INSIGHTS.map((post) => (
            <div
              ...
              className="p-6 rounded-2xl bg-slate-950/80 border border-slate-800 hover:border-slate-700 transition-all flex flex-col justify-between cursor-pointer group hover:shadow-xl"
            >
              <div>
                {/* Meta details */}
      ...
      {selectedArticle && (
        ...
        >
          <div
            className="w-full max-w-2xl bg-slate-900 border border-slate-700 rounded-2xl p-6 sm:p-8 text-slate-100 space-y-6 shadow-2xl"
